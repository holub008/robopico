...
# ignore list
with open(os.path.join(rpd.DATA_ROOT, 'minimap', 'ignorelist.txt'), 'r') as f:
    ignores = set((line.strip() for line in f))

with open(os.path.join(rpd.DATA_ROOT, 'minimap', 'str_to_cui.pck'), 'rb') as f:
    str_to_cui = pickle.load(f)

with open(os.path.join(rpd.DATA_ROOT, 'minimap', 'cui_to_mh.pck'), 'rb') as f:
...
with open(os.path.join(rpd.DATA_ROOT, 'minimap', 'prepositions_conjunctions.txt'), 'r') as f:
    prep_conj = [line.strip() for line in f]
    ...
    if abbrevs:
        for abbrev, expansion in abbrevs.items():
            try:
                ...
                # to avoid weird errors in abbreviations generating error causing regex strings
                # (which are not causing a named exception)
                print(f"Regex error caused for one abstract! (for text string '{text_str}')", file=sys.stderr)
                print(f"and abbreviation dictionary '{abbrevs}'", file=sys.stderr)
                continue
